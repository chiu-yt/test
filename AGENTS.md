...
## Research Context
- This repo is now being used for a **single-agent multimodal BEVFusion adaptation** study whose first-paper mainline is **normal-scene deployment shift**, not adverse-weather online TTA.
- Read `Experiment Record.md` and `docs/guidelines_of_approaches/bevfusion_multimodal_normal_shift_plan.md` before making nontrivial research changes.
- The current first-paper objective is: **within the existing nuScenes + BEVFusion codepath, develop a lightweight multimodal adaptation method that reuses the current engineering stack and yields a stable, publishable gain without requiring a new codebase or source-side retraining.**
- Keep the project constrained to **single-agent BEVFusion**. Do not silently expand into collaborative / V2X / DAIR-V2X-C / V2V4Real directions unless the task explicitly approves a new dataset stack and substantial infrastructure work.
- The repo's mature multimodal path is `nuScenes + BEVFusion`; cross-dataset or collaborative multimodal adaptation should be treated as follow-on work, not the assumed default.
- The preferred first-paper method families are, in order: `BEVFusion-native cross-modal reliability calibration`, `parameter-efficient fusion adapters`, and `backprop-free checkpoint/model merging`.
- Keep the current non-finite pseudo-label / loss protections enabled unless a task explicitly re-evaluates them; they remain part of the reusable engineering baseline.
- Retain `F1 + D1.3` as the strongest archived adverse-weather baseline and as a source of reusable stabilization heuristics, not as the main paper target.
- The main bottleneck observed in the archived adverse-weather line was noisy pseudo labels, especially `pedestrian` and `traffic_cone`, rather than a total detector-backbone failure.
...
- Keep `GaussianLSS`, evidential learning heads, dual-expert/PanDA refinements, and collaborative/V2X UDA inheritance out of the first-paper mainline unless a task explicitly expands scope to architecture redesign or a new codebase.
- Archived adverse-weather findings remain important research assets. The key result is that under strong fog, fused dual-modality predictions can underperform single-modality baselines, and the `fog_s3_conflict_probe` showed that symmetric `B_L`/`B_C` center-distance conflict is not a useful reliability signal (`overall=-0.034`, `pedestrian=-0.014`, `traffic_cone=-0.010`).
- The archived fog line should be preserved for later work on **asymmetric modality collapse**, `LiDAR-anchored geometry verification`, and raw low-score pseudo-label diagnostics.
