...
- Do not make checkpoint aggregation the paper center; prioritize **LiDAR-anchored, parameter-free pseudo-label reliability modeling** using LiDAR geometry verification (`point_count`, `point_density`, `z_span` / `z_var`) over additional aggregation complexity or fine-grained threshold retuning. Frame/object LSS depth entropy probes were weak and mixed, so entropy should be treated as a secondary cue rather than the next mainline.
...
- The current one-sentence problem is: **under adverse-weather asymmetric modality degradation, how can source-free BEVFusion TTA directly verify and reuse low-confidence fused pseudo labels using LiDAR physical geometry instead of relying on fused score, camera entropy, or symmetric LiDAR-camera agreement?**
- Latest `fog_s3_raw_window_diag` result: `pedestrian` has a small raw relaxed window (`raw_total=419`, `raw_after_neg=16`, `raw_relaxed_window=11`, `raw_above_score=5`) but `ignored_relaxed=0` and `promoted=0`; `traffic_cone` has `raw_total=12` and all are below `NEG_THRESH`. This means the next method should not tune `point_density_min` / `z_span_min` yet; geometry verification must move to **direct raw-proposal promote/reweight** before `memory_ensemble_utils.save_pseudo_label_batch()` drops the relaxed candidates.
- The advisor-endorsed framing is reasonable if written conservatively: treat direct raw-proposal promote/reweight as **Physical-over-Semantic recall mining** to break pseudo-label confirmation bias, but implement it as soft **Geometric Confidence Calibration** (`R_geo` from sigmoid-mapped density / z-span / optional point count) rather than a brittle hard-threshold trick.
- Latest `fog_s3_raw_geometry_direct_promote_ped_soft` result shows the new chain is live (`raw_relaxed_window -> geometry_checked -> promoted`), but `Pred[pedestrian]` inflates to ~`5e5` and overall `NDS/mAP` stay roughly flat. Treat this as evidence that **candidate dedup + raw_direct_reweight** should become the new mainline, with direct promote demoted to an ablation.
- Next method landing should be staged: implement stricter low-score candidate dedup / TTA-NMS before geometry scoring, switch the main method to `raw_geometry_direct_reweight`, validate candidate-count reduction and TP/FP proxy quality on `fog s3`, then run short TTA on `fog s3`, and confirm on `fog s5` only after `s3` improves. Use `traffic_cone` low-score extraction only after `pedestrian` proves the reweight path.
