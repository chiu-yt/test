...
- Next method landing should be staged: implement `raw_geometry_direct_promote` / `raw_geometry_direct_reweight` for target-class raw proposals, first validate promoted-count and TP/FP proxy quality on `fog s3`, then run short TTA on `fog s3`, and confirm on `fog s5` only after `s3` improves. Use `traffic_cone` low-score extraction only after `pedestrian` proves the direct raw-proposal path.
