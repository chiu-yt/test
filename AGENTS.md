...
- Next method landing should be staged: first run a `fog s3` pseudo-label quality probe with `roiaware_pool3d` for fused pseudo boxes, comparing TP/FP proxy distributions of `point_count`, `point_density`, `z_span`, and `z_var` for `pedestrian` / `traffic_cone`; only then test point-density-only or combined geometry+entropy reliability, run TTA on `fog s3` if pseudo-label quality improves, and confirm on `fog s5` only after `s3` succeeds.
